...
class ThreatDetectionPipeline:
    ...
    def load_models(self, isolation_forest_path: Optional[str] = None,
        ...
        if isolation_forest_path and Path(isolation_forest_path).exists():
            ...
            self.isolation_forest.model_version=_hashlib.sha256(Path(isolation_forest_path).read_bytes()).hexdigest()[:16]
        ...
        if random_forest_path and Path(random_forest_path).exists():
            ...
            import hashlib as _hashlib
            self.random_forest.model_version=_hashlib.sha256(Path(random_forest_path).read_bytes()).hexdigest()[:16]
    ...
    def process_pcap(self, pcap_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.stats["detections_found"]=sum(sum(len(v) for v in w["detections"].values()) for w in results["windows"])
