...
class RandomForestThreatClassifier:
    ...
    def predict(self, flows: List[FlowRecord]) -> List[Dict[str, Any]]:
        ...
        expected=list(FEATURE_SCHEMA.feature_names)
        if list(self.feature_columns or expected) != expected:
            raise ValueError("MODEL_SCHEMA_MISMATCH: model feature order does not match current feature schema")
        # Extract features
        X = self.extract_features(flows)
    ...
    def load_model(self, filepath: str) -> bool:
        ...
        try:
            ...
            from src.features.schema import FEATURE_SCHEMA
            if model_data.get("feature_schema_version") not in (None, FEATURE_SCHEMA.schema_version):
                raise ValueError(f"model feature schema mismatch: {model_data.get('feature_schema_version')} != {FEATURE_SCHEMA.schema_version}")
